...
class WDMSimulatorPhD:
    ...
    def simulate_network_real(self, load_values: List[int] = None, 
                            ...
                            output_file: str = "simulation_phd_results.txt") -> Dict[str, List[float]]:
        """
        ...
        # Abre arquivo para escrever resultados
        with open(output_file, 'w') as f:
            ...
            f.write("# " + "="*60 + "\n")
            f.write("# Formato: Simulação Load Probabilidade_Bloqueio_Tempo_Execução(s)\n")
            f.write("# " + "="*60 + "\n\n")
        ...
        for sim in range(num_simulations):
            ...
            # Executa algoritmo genético uma vez por simulação
            ...
            for load_idx, load in enumerate(load_values):
                ...
                blocked_calls = 0
                total_calls = 1000  # 1000 chamadas por load
                ...
                for call_id in range(total_calls):
                    ...
                    source, target = self.manual_pairs[pair_idx]
                    
                    # Obtém a rota escolhida pelo AG para este par
                ...
                all_results[load].append(blocking_prob)
                
                # Tempo de execução deste load
                load_time = time.time() - load_start_time
                
                # Salva no arquivo
                with open(output_file, 'a') as f:
                    f.write(f"{sim + 1} {load} {blocking_prob:.6f} {load_time:.2f}\n")
                
                # Progresso
                if (load_idx + 1) % 20 == 0 or load_idx == len(load_values) - 1:
                    print(f"    Load {load}: {blocking_prob*100:.2f}% bloqueio ({load_time:.2f}s)")
            
            # Tempo total desta simulação
            sim_time = time.time() - sim_start_time
            print(f"  Tempo da simulação {sim + 1}: {sim_time:.2f} segundos")
            
            # Salva tempo no arquivo
            with open(output_file, 'a') as f:
                f.write(f"# Tempo simulação {sim + 1}: {sim_time:.2f}s\n")
        ...
        for load in load_values:
            if all_results[load]:
                avg_blocking_by_load[load] = np.mean(all_results[load])
        ...
        with open(output_file, 'a') as f:
            ...
            f.write(f"# Média de tempo por simulação: {total_time/num_simulations:.2f}s\n")
            
            if self.execution_times:
                f.write(f"# Tempo médio do AG: {np.mean(self.execution_times):.2f}s\n")
            
            f.write("\n# Probabilidade de Bloqueio Média por Load:\n")
            f.write("# Load  Probabilidade_Bloqueio_Média\n")
            for load in sorted(avg_blocking_by_load.keys()):
                f.write(f"{load:4d}  {avg_blocking_by_load[load]:.6f}\n")
        
        print(f"\n{'='*60}")
        print("SIMULAÇÃO CONCLUÍDA")
        print(f"{'='*60}")
        print(f"Tempo total: {total_time:.2f}s ({total_time/60:.2f} minutos)")
        print(f"Média por simulação: {total_time/num_simulations:.2f}s")
        print(f"Resultados salvos em: {output_file}")
        
        # Gera gráfico
        self._plot_results(load_values, avg_blocking_by_load, num_simulations)
        
        return dict(all_results)

    def _plot_results(self, load_values: List[int], avg_blocking_by_load: Dict[int, float],
                     num_simulations: int) -> None:
        """
        Gera gráfico dos resultados.
        
        Args:
            load_values: Lista de valores de carga
            avg_blocking_by_load: Dicionário com probabilidades médias por load
            num_simulations: Número de simulações realizadas
        """
        plt.figure(figsize=(12, 8))
        
        # Extrai dados para o gráfico
        loads = sorted(avg_blocking_by_load.keys())
        probs = [avg_blocking_by_load[load] for load in loads]
        
        # Gráfico principal
        plt.plot(loads, probs, 'b-', linewidth=2.5, label='Probabilidade de Bloqueio')
        plt.fill_between(loads, probs, alpha=0.2, color='blue')
        
        # Destaca pontos a cada 20 loads
        highlight_loads = loads[::20]
        highlight_probs = [avg_blocking_by_load[load] for load in highlight_loads]
        plt.scatter(highlight_loads, highlight_probs, color='red', s=80, 
                   zorder=5, label='Pontos de amostragem')
        
        # Configurações do gráfico
        plt.xlabel('Carga de Tráfego (Load)', fontsize=14)
        plt.ylabel('Probabilidade de Bloqueio', fontsize=14)
        plt.title(f'Simulação WDM - Algoritmo Genético (Doutorado)\n{num_simulations} simulações, {self.num_wavelengths} wavelengths', 
                 fontsize=16, fontweight='bold')
        plt.grid(True, alpha=0.3, linestyle='--')
        plt.legend(fontsize=12)
        
        # Formatação dos eixos
        plt.xlim(min(loads) - 5, max(loads) + 5)
        plt.ylim(-0.02, min(1.0, max(probs) * 1.2))
        plt.xticks(np.arange(min(loads), max(loads) + 1, 20))
        
        # Adiciona texto informativo
        info_text = f'Configuração:\n• Pares O-D: {len(self.manual_pairs)}\n• Simulações: {num_simulations}\n• Chamadas/load: 1000'
        ...
        # Salva o gráfico
        plot_filename = f'simulation_phd_{time.strftime("%Y%m%d_%H%M%S")}.png'
...
def main():
    ...
    # Executa simulação REAL
    print("\nIniciando simulação real da rede...")
    results = wdm_simulator.simulate_network_real(
        load_values=list(range(1, 201)),  # Loads de 1 a 200
        num_simulations=20,  # 10 simulações independentes
        output_file="simulation_results_phd_detailed.txt"
    )
    ...
    # Salva arquivo final com tempo total
    with open("tempo_execucao_phd.txt", "w") as time_file:
        ...
        time_file.write(f"Simulações realizadas: 10\n")
        time_file.write(f"Loads testados: 200 (1-200)\n")
        time_file.write(f"Chamadas por load: 1000\n")
        time_file.write(f"Total de chamadas simuladas: 10 * 200 * 1000 = 2,000,000\n")
    ...
    print(f"  • simulation_results_phd_detailed.txt - Resultados detalhados")
