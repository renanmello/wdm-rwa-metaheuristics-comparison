...
class WDMSimulator:
    ...
    def _fitness_route(self, route: List[int]) -> float:
        """
        Calcula a aptidão de uma rota específica APENAS baseada em características da rota.
        ...
        # Número de saltos (quanto menor, melhor)
        hops = len(route) - 1
        
        # Penaliza rotas com mais saltos (inverso)
        hops_score = 1.0 / (hops + 1)
        
        # Para continuidade de wavelength: assumimos que sempre tentamos manter o mesmo wavelength
        continuity_score = 1.0  # Máximo, já que não estamos simulando alocações no fitness

        # Função de fitness combinada
        fitness = (self.hops_weight * hops_score + 
                   self.wavelength_weight * continuity_score)

        return fitness

    def _fitness(self, individual: List[int], source_targets: List[Tuple[int, int]]) -> float:
        """
        Calcula a aptidão de um indivíduo baseado apenas nas características das rotas.
    ...
    def simulate_network(
        ...
        print(f"\n{'='*50}")
        print(f"EXECUTANDO DE PARA ENCONTRAR MELHOR SOLUÇÃO")
        print(f"{'='*50}")
        ...
        for gene_idx, file in route_files.items():
            file.close()
            source, target = self.manual_pairs[gene_idx]
            print(f"✓ Resultados da rota [{source},{target}] salvos em route_{source}_{target}_de.txt")
    ...
    def _save_statistics(self, all_simulations: Dict[int, List[List[float]]], num_sims: int) -> None:
        ...
        for gene_idx in range(self.gene_size):
            ...
            # Arquivo de médias
            mean_filename = f"gene_{gene_idx + 1}_de_mean.txt"
            ...
            std_filename = f"gene_{gene_idx + 1}_de_std.txt"
            ...
            # Arquivo combinado
            combined_filename = f"gene_{gene_idx + 1}_de_stats.txt"
    ...
    def _save_execution_time(self, total_time: float, num_sims: int) -> None:
        ...
        """
        filename = "execution_time_de.txt"
        with open(filename, "w") as f:
            f.write("# Tempo de Execução - DE\n")
    ...
    def plot_individual_genes(
        self, 
        save_path: str = "grafico_wdm_simulation_de.png"
    ) -> None:
        """
        ...
        plt.title('Probabilidade de Bloqueio por Rota (DE)', fontsize=14)
    ...
    def generate_comparison_table(
        self, 
        output_file: str = "comparacao_rotas_de.csv"
    ) -> pd.DataFrame:
        """
        ...
        print("\nTabela de Comparação (primeiras 10 cargas):")
...
def main():
    ...
    # Executa simulação
    output_file = f"blocking_results_de_{num_wavelengths}w.txt"
    ...
    wdm_simulator.plot_individual_genes(
        save_path=f"grafico_wdm_de_{num_wavelengths}w.png"
    )
    wdm_simulator.generate_comparison_table(
        output_file=f"comparacao_rotas_de_{num_wavelengths}w.csv"
    )
